...
2. ~~Add a sitemap and robots.txt.~~ **DONE 2026-07-07** (commit `c9e9e82`): `src/app/sitemap.ts` (all routes) + `src/app/robots.ts` (all crawlers incl. AI bots allowed).
3. ~~Set the canonical address.~~ **DONE 2026-07-07** (same commit): `metadataBase` + canonical in `layout.tsx`. **One step remains when the domain is purchased: set `NEXT_PUBLIC_SITE_URL=https://komponentguiden.se` in Vercel env (Production) and redeploy** — sitemap, robots and canonicals all switch automatically. Until then they point at vercel.app (correct for now).
