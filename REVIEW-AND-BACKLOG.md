...
24. **ScrollyTelling step 4 rework** — prompt already written in the GTM session, not yet executed — **and a mobile fallback** (the component currently renders nothing on mobile, so phone visitors miss the entire "how it works" story).
...
- **ScrollyTelling (24):** `src/components/ScrollyTelling.tsx` is desktop-only (hidden on mobile, 400vh scroll container). Step-4 rework prompt exists from the owner's GTM session (ask owner for it — not in the repo). Mobile fallback: render the four stages as a static stacked list under a media query instead of nothing.
