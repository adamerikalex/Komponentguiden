...
3. **Fix the numbers story.** The animated counters start at fake values (1 000 / 10 / 0) and count up via JavaScript — but Google reads the page *before* the animation runs, so it records the fake values. Meanwhile the hero text claims "tusentals kartlagda företag" while the counter says 171 — a contradiction on one scroll. Fix: put the real numbers directly in the page (animation as decoration only), align the hero claim, and hide "0 presenterade matchningar" until it's not zero.
